...
const WeatherDashboard = () => {
  ...
  const getWeatherIcon = (weatherCondition) => {
    switch (weatherCondition?.toLowerCase()) {
      ...
      case "snow":
        return <WiSnow size={40} color="#F0FFFF" />;
      case "thunderstorm":
        return <WiThunderstorm size={40} color="grey" />;
      case "fog":
      case "mist":
        return <WiFog size={40} color="#B0C4DE" />;
      default:
        return <WiDaySunny size={40} color="yellow" />; // Default to sunny
    }
  };

  const fetchWeatherData = useCallback(async (cityOrLat, lon) => {
    setLoading(true);
    try {
        let lat, city;
        
        if (lon) {
          // If longitude is provided, then we're using geolocation
          lat = cityOrLat;
        } else {
          // If longitude is not provided, treat cityOrLat as city name
          city = cityOrLat;
          const coordsResponse = await axios.get(
              `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${API_KEY}`
          );
          lat = coordsResponse.data.coord.lat;
          lon = coordsResponse.data.coord.lon;
        }
  ...
  const getCurrentLocation = useCallback(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        fetchWeatherData(latitude, longitude); // Pass latitude and longitude directly
      }, (error) => {
